...
output reg rd_req;//read request from axi
output reg[`AXI_ADDR_WIDTH-1:0]	rd_addr;//the read address
output reg[7:0]	rd_len;//the length of one read burst
input rd_finish;//finishing reading data
output reg wr_req;//write request to axi
output reg[`AXI_ADDR_WIDTH-1:0]	wr_addr;//write address
output reg[7:0]	wr_len;//write burst length
...
output	reg	image_rd_finish;//finishing reading all data from axi
...
reg [11:0]	inf_write_read_control;
...
//					inf_write_read_control	<=	inf_write_read_iterate_head;
			...
			inf_write_read_head_rd:
				if(rd_finish)//in this state, reading head is in operation
...
reg rd_req_temp1, rd_req_temp2;

reg wr_req_temp;//write request temparory
reg wr_req_temp1, wr_req_temp2;
...
reg [`AXI_ADDR_WIDTH-1:0] head_addr;
...
begin
		...
		else if(inf_write_read_control == inf_write_read_head_rd && rd_finish)//1 burst
...
begin
		...
		else if(inf_write_read_control == inf_write_read_image_rd && rd_finish)
...
begin
		...
		else if(inf_write_read_control == inf_write_read_kernel_rd && rd_finish)
//			if(kernel_addr[7:0] + sub_result > 9'h100)
//			if(({1'b0,kernel_addr[8:0]} + {2'b0,sub_result[7:0]}) > 10'b1000_0000_00)//boundary
//				kernel_addr <= {kernel_addr[`AXI_ADDR_WIDTH-1:8] + 1'b1, 8'h0} + sub_result;
//				kernel_addr	<=	{kernel_addr[`AXI_ADDR_WIDTH-1:0] + 10'b1000_0000_00};
//			else
//				kernel_addr	<=	kernel_addr + sub_result;
				kernel_addr	<=	{kernel_addr + 10'b1000_0000_00};
end
...
//////////////////////////////////write address//////////////////////////////////////////////////////  
reg [`ADDR_HIGH_BIT_WIDTH-1:0]	image_wr_high_addr;
reg [`ADDR_LOW_BIT_WIDTH-1:0]	image_wr_low_addr;  
...
begin
		if(image_begin_pos_r || one_layer_finish_r1)
			{image_wr_high_addr, image_wr_low_addr}	<=	image_base_wr_addr;
		else if(inf_write_read_control == inf_write_read_image_wr && wr_finish)
			{image_wr_high_addr, image_wr_low_addr}	<=	{image_wr_high_addr, image_wr_low_addr} + sub_result;
end

always @(posedge clk)
begin
		wr_addr	<=	{image_wr_high_addr, image_wr_low_addr,3'b000};
end
...
always @(posedge clk)
begin
//	if(({1'b0,rd_addr_temp[8:0]} + {2'b0,sub_result[7:0]}) > 10'b1000_0000_00)//boundary
//		rd_addr	<=	{rd_addr_temp + 10'b1000_0000_00, 3'b000};
//    else        //normal
        rd_addr	<=	{rd_addr_temp, 3'b000};
end
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
reg [7:0]	wr_finish_cnt;
reg [7:0]	rd_finish_cnt;
...
begin
		if(!rst_n)
			wr_finish_cnt	<=	8'h0;
		else if(wr_finish_cnt == finish_cnt_threshold && wr_finish)
			wr_finish_cnt	<=	8'h0;
		else if(wr_finish)
...
begin
		if(!rst_n)
			rd_finish_cnt	<=	8'h0;
		else if(rd_finish_cnt == finish_cnt_threshold && rd_finish)
			rd_finish_cnt	<=	8'h0;
		else if(rd_finish && (information_operation_temp[2] || information_operation_temp[3]))
...
assign all_rd_finish	=	(rd_finish_cnt == finish_cnt_threshold && rd_finish) ? 1'b1 : 1'b0;
assign all_wr_finish	=	(wr_finish_cnt == finish_cnt_threshold && wr_finish) ? 1'b1 : 1'b0;
//////////////////////////////////////////////////////////////////////////////////////////////////////////
