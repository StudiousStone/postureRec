...
module iterate_head(
	...
	output image_en,//whether to read from axi--表示当前图像操作
	output image_next_load,//表示下一个head信息是需要从DDR中读取图像的
	...
	output [`KERNEL_NUM-1:0]	kernel_num,//kernel number in one filter--当前操作中kernel的数量
	output [`KERNEL_SIZE-1:0]	kernel_size,//kernel size
	output [`FILTER_NUM-1:0]	filter_num//filter number--当前操作中filter的数量
