...
reg		[`IMAGE_SIZE-1:0]	image_wr_cnt;//counter for write image to fifo
...
always @(posedge clk)
begin
	if(compare_final_data_cnt == maxpool_image_pixelNum - 1'b1 && compare_valid && maxpooling_flag)
...
always @(posedge clk)
begin
	if(compare_final_data_cnt == maxpool_image_pixelNum - 1'b1 && compare_valid && image_size_after_maxpooling[0] && maxpooling_flag)
...
reg		[`IMAGE_SIZE-1:0]	image_rd_cnt;
reg		[`IMAGE_SIZE-1:0]	rd_allimage_cnt;//count the image data for read
wire    allimage_rd_finish1;
wire	allimage_rd_finish2;
reg		[`IMAGE_SIZE*2-1:0] image_noMax_cnt;
